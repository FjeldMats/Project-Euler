...
it will either (i) become a palindrome in less than
...
def iterate(n):
    num1 = n
    num2 = int(''.join(map(str, list(reversed(list(str(n)))))))
    return num1 + num2
...
def isLychrel(n):
    ...
    while not isPalindromic(number):
        number = iterate(number)
        iterations += 1
        if(iterations >= 50):
            return True
    #print str(number) + " (itr: "+ str(iterations)+")"
    return False
